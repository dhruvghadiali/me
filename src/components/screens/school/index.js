...
const SchoolScreenComponent = (props) => {
  const dispatch = useDispatch();

  const { callPublicAPI } = props;
  ...
  const handleCardClick = (schoolId) => {
    dispatch(getSchool({ schoolId, callPublicAPI: callPublicAPI || false }));
    if (isFullScreen) {
  ...
  return (
    <>
      {schoolSummaryLoader ? (
        ...
        <SchoolsNotFound callPublicAPI={callPublicAPI} />
      ) : (
...
export default SchoolScreenComponent;
